...
% Given initialization + projection
    ...
    [U,B] = poldec(A_ls); % Y = K*X, so K = Y * pinv(X);
    try
        eA = abs(eigs(U*B, 1, 'lm', 'MaxIterations', 3000, 'SubspaceDimension', n));
    catch
    ...
    if eA > 1
        ...
        e_old = norm(Y - (S\ U * B* S) * X, 'fro')^2/2;
    else
...
%     eA = abs(eig(A_ls)); 
    maxeA = max(1,eA); 
    Astab = (A_ls)/maxeA; 
    [~,Stemp,Utemp,Btemp] = checkdstable(0.9999*Astab); 
    etemp = norm(Y - (Stemp\ Utemp * Btemp* Stemp) * X, 'fro')^2/2;
    if etemp < e_old
        S = Stemp; 
        U = Utemp;
        B = Btemp;
...
while i < options.maxiter 
    ...
    step = 100;
        ...
        try
            maxE = abs(eigs(Un*Bn, 1, 'lm', 'MaxIterations', 3000, 'SubspaceDimension', n));
        catch
        ...
        e_new = norm(Y - (Sn\ Un * Bn* Sn) * X, 'fro')^2/2;
...
function [P,S,U,B] = checkdstable(A) 
